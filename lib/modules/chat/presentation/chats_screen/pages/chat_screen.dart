...
class ChatScreenState extends State<ChatScreen> implements ChatChooseDelegate {
        ...
        chatDataSource: ChatDataSourceImpl(
          ...
          multipartRequest: http.MultipartRequest(
              ...
              Endpoints.sendMessages
                  .buildURL(urlParams: [widget.chatEntity.chatId.toString()])),
        ),
                ...
                child: BlocConsumer<ChatBloc, ChatState>(
                  ...
                  return Scaffold(
                    appBar: this.buildAppBar(
                        _chatTodoCubit, cubit, state, chatViewModel, navigator,
  ...
  bool canSendMessage(ChatState state) {
    return !(state.chatEntity.permissions?.adminMessageSend ?? false) ||
        isAdmin(state);
  }

  bool canSendMedia(ChatState state) {
    print((state.chatEntity.permissions?.isMediaSendOn ?? true) ||
        isAdmin(state));
    return (state.chatEntity.permissions?.isMediaSendOn ?? true) ||
        isAdmin(state);
  }
